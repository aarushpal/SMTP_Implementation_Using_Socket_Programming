...
// server.smtp.com is the domain name of the server
// 25 is the port number of the server
...
void send_image(int socket)
{
    ...
    //feof is a macro that returns true if the end of file has been reached
    while(!feof(fpimage))
    {
...
char * Mail_Header(char *from,char *to,char *sub,char *content)
{
    ...
    /*localtime converts a time_t value to calendar time and returns a pointer to a 
    tm structure with its members filled with the corresponding values*/
    //strftime converts the broken-down time, expressed as local time, to a string
    // sprintf is used to write formatted output to a string
    ...
    //copying all the header parts to the header variable
...
{
    //struct hostent *host is a structure that will get information about the host
    //struct sockaddr_in server_addr is a structure that will contain an internet address
    int socket_id,n;
    int portno;
    ...
    do
    {
        ...
        // It is the first command that a client sends after establishing a TCP connection
        // It is used to start the SMTP conversation
        if(strcasecmp(cname,"HELO") == 0)
        {
            ...
            // fflush is used to clear the output buffer and move its content to console
            fflush(stdin);
        }
        ...
        // RCTP TO command
        // this command is used to send the email to the receiver

        //strcasecmp is used to compare two strings without considering the case
        ...
        // DATA command
        // This command is used to send the actual message to the receiver
        ...
        // ATTACHMENT command
        // This command is used to send the attachment to the receiver
